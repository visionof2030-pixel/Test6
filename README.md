    ...
    <style>
        ...
        * {
        ...
        .top-bar {
            ...
            box-shadow: 0 4px 30px rgba(240, 180, 41, 0.15);
            position: relative;
            overflow: hidden;
            background: #0a1628;
        }
        ...
        .name-modal-content .user-status .highlight {
            font-weight: 800;
        }
        ...
        .predict-btn-wrap .stream-btn:hover {
        ...
        .round-section:hover {
            border-color: var(--gold);
            box-shadow: 0 8px 40px rgba(240, 180, 41, 0.08);
        }
        .round-title {
            font-size: 1.2rem;
            font-weight: 800;
        ...
        .bracket-match .teams .team:last-child {
            justify-content: flex-end;
        }
        .bracket-match .teams .team .flag {
            font-size: 1.2rem;
        }
        ...
        .bracket-match .match-status .highlight {
            color: var(--gold-light);
            font-weight: 700;
        }
        .bracket-match .match-status .finished-status {
            color: var(--success);
        }
        .bracket-match .match-status .pending-status {
            color: var(--text-secondary);
        }
        .bracket-match .match-status .live-status {
            color: var(--danger);
            animation: pulseText 1.2s infinite;
        }
        .bracket-match .match-time-badge {
            font-size: 0.55rem;
            color: var(--text-secondary);
            text-align: center;
            opacity: 0.7;
        ...
        .stream-modal-content .stream-title {
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-bottom: 12px;
        }
        .stream-modal-content .stream-title h2 {
            font-size: 1.1rem;
            color: var(--gold-light);
        }
        .stream-modal-content .stream-title .close-btn {
            background: none;
            border: none;
            color: var(--text-secondary);
            font-size: 1.6rem;
        ...
        .stream-modal-content .stream-title .close-btn:hover {
            color: var(--danger);
        }
        .stream-modal-content video {
            width: 100%;
            max-height: 70vh;
            background: #000;
            border-radius: var(--radius-md);
        }
        .stream-modal-content .stream-error {
            color: var(--danger);
            text-align: center;
            padding: 12px;
            font-weight: 600;
        ...
        .stream-modal-content .stream-msg {
            color: var(--text-secondary);
            text-align: center;
            padding: 8px;
            font-size: 0.8rem;
        }
        .stream-modal-content .stream-controls {
            display: flex;
            gap: 10px;
            justify-content: center;
            margin-top: 10px;
        }
        .stream-modal-content .stream-controls button {
            background: rgba(255, 255, 255, 0.06);
            border: 1px solid rgba(255, 255, 255, 0.08);
            color: var(--text-secondary);
            padding: 6px 16px;
        ...
        .stream-modal-content .stream-controls button:hover {
            background: rgba(240, 180, 41, 0.12);
            border-color: var(--gold);
            color: var(--gold-light);
        }
...
<body>
    ...
    <div class="app-container">
        ...
        <div class="news-ticker-wrapper" id="newsTickerWrapper">
            <div class="news-ticker" id="newsTicker">
                ...
                <span class="sep">|</span>
                <span>🗳️ طريقة التوقع: اختر المباراة → أدخل اسمك → اختر الفائز أو التعادل</span>
                <span class="sep">|</span>
        ...
        <div class="admin-controls" id="adminControls">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
                ...
                <button class="admin-btn success" onclick="openAnalytics()">📊 تحليلات</button>
        ...
        <div class="tabs-container">
            <div class="tabs">
                ...
                <button class="tab-btn" data-tab="bracket">🏆 مسار البطولة</button>
        ...
        <div id="bracketTab" class="tab-content">
            <div class="section-title">🏆 مسار البطولة</div>
    ...
    <!-- نافذة عرض توقعات المباراة السابقة + تفاصيل الأهداف -->
    ...
    <!-- نافذة التوقع (الخطوة الثانية - تظهر بعد التحقق من الاسم) -->
    ...
    <div class="modal-overlay" id="playerPredictionsModal">
        <div class="modal-content">
            ...
            <div style="text-align:center;font-size:0.8rem;color:var(--text-secondary);margin:4px 0 12px;">
                إجمالي التوقعات: <span id="playerTotalCount" style="color:var(--gold-light);font-weight:800;">0</span>
            </div>
            ...
            <div id="analyticsContent">
                <div class="analytics-grid">
                    <div class="chart-box full-width">
                        <div class="chart-title">📊 توزيع التوقعات لكل مباراة</div>
    ...
    <!-- ===== نافذة البث المباشر (IPTV Player) ===== -->
    <div class="modal-overlay" id="streamModal">
        <div class="modal-content stream-modal-content">
            <div class="stream-title">
                <h2>📺 البث المباشر</h2>
                <button class="close-btn" onclick="closeStreamModal()">✕</button>
            </div>
            <div id="streamPlayerContainer">
                <video id="streamVideo" controls autoplay></video>
                <div class="stream-error" id="streamError">⚠️ فشل تحميل البث. يرجى المحاولة لاحقاً.</div>
                <div class="stream-msg" id="streamMsg">جاري تحميل البث...</div>
                <div class="stream-controls">
                    <button onclick="retryStream()">🔄 إعادة المحاولة</button>
                    <button onclick="closeStreamModal()">❌ إغلاق</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // ============================================================
        //  SAUDI TIME helpers - now using local timezone (user's device)
        // ============================================================
        function toSaudiTime(isoString) {
            // Return a Date object from ISO string, respecting the timezone offset in the string
            return new Date(isoString);
        }

        function getSaudiNow() {
            // Return current local time (user's device)
            return new Date();
        }
        ...
        // ============================================================
        //  BEIN SPORTS CHANNELS (IPTV)
        // ============================================================
        const beinChannels = [
            "http://amir11.bounceme.net:8080/YZjwcwnLjh/QlXrgJTnED/1127", // Bein Max 1
            "http://amir11.bounceme.net:8080/YZjwcwnLjh/QlXrgJTnED/1130", // Bein Max 2
            "http://amir11.bounceme.net:8080/YZjwcwnLjh/QlXrgJTnED/1132", // Bein Max 3
            "http://amir11.bounceme.net:8080/YZjwcwnLjh/QlXrgJTnED/1133" // Bein Max 4
        ];

        // ============================================================
        //  Stream Player (HLS.js)
        // ============================================================
        let hls = null;

        function openStreamModal(url, matchTitle) {
            const modal = document.getElementById('streamModal');
            const video = document.getElementById('streamVideo');
            const errorDiv = document.getElementById('streamError');
            const msgDiv = document.getElementById('streamMsg');

            // Reset UI
            errorDiv.style.display = 'none';
            msgDiv.style.display = 'block';
            msgDiv.textContent = '⏳ جاري تحميل البث...';
            ...
            if (typeof Hls === 'undefined') {
                ...
                errorDiv.textContent = '❌ مشغل البث غير متوفر. يرجى تحديث المتصفح.';
            ...
            if (video.canPlayType('application/vnd.apple.mpegurl')) {
                ...
                video.play().catch(() => {
                    errorDiv.style.display = 'block';
                    errorDiv.textContent = '⚠️ تعذر تشغيل البث. قد يكون الرابط غير صحيح أو البث غير متاح.';
                    msgDiv.style.display = 'none';
                });
                return;
            }
            ...
            try {
                ...
                hls.on(Hls.Events.MANIFEST_PARSED, function() {
                    msgDiv.textContent = '✅ البث جاهز، جاري التشغيل...';
                    video.play().catch(() => {
                        // Autoplay blocked, user must click play
                        msgDiv.textContent = '▶️ اضغط على زر التشغيل لمشاهدة البث.';
                    });
                });
                ...
                hls.on(Hls.Events.ERROR, function(event, data) {
                    if (data.fatal) {
                        errorDiv.style.display = 'block';
                        errorDiv.textContent = '❌ حدث خطأ في البث. قد يكون الرابط غير صحيح أو الخادم غير متاح.';
                        msgDiv.style.display = 'none';
                    }
                });
                ...
                errorDiv.style.display = 'block';
                errorDiv.textContent = '❌ فشل تهيئة المشغل: ' + e.message;
                msgDiv.style.display = 'none';
        ...
        function retryStream() {
            // Get current stream URL from video element or stored data
            const video = document.getElementById('streamVideo');
            const url = video.src;
            if (url) {
                // Close and re-open
                ...
                showCopyToast('⚠️ لا يوجد رابط بث لإعادة المحاولة');
            }
        }
        ...
        // ============================================================
        //  دوال مساعدة للوقت (بتوقيت جهاز المستخدم)
        // ============================================================
        ...
        // ============================================================
        //  الترجمة والأعلام (نفس الكود السابق)
        // ============================================================
        ...
        // توزيع روابط البين سبورت على المباريات بشكل دوري
        const matchesData = rawMatches.map((m, index) => ({
            ...m,
            timeISO: m.time + "+03:00",
            roundLabel: m.round === 'first' ? 'الجولة الأولى' : (m.round === 'second' ? 'الجولة الثانية' : 'الجولة الثالثة'),
            streamUrl: beinChannels[index % beinChannels.length]
        }));
        ...
        // ============================================================
        //  دالة مساعدة للعثور على نتيجة المباراة
        // ============================================================
        function findMatchResult(team1, team2) {
            let match = state.previousGamesData.find(m => (m.homeAr === team1 && m.awayAr === team2) || (m.homeAr === team2 &&
                    m.awayAr === team1));
            if (match) return { homeScore: match.homeScore, awayScore: match.awayScore, homeAr: match.homeAr, awayAr: match
                    .awayAr };
        ...
        function renderMatchCard(m, isUpcoming) {
            ...
            // التحقق مما إذا كانت المباراة في توقعات المستخدم
            ...
            // إذا كان المستخدم قد توقع هذه المباراة
            if (userHasPrediction) {
                ...
                // توقع سابق (ربما مستخدم آخر) لا نعرضه
                predictDisabled = true;
                predictBtnHtml = 'تم التوقع ✅';
                predictBtnClass += ' submitted';
                predictBtnExtra = 'disabled';
            ...
            // زر البث المباشر - يفتح النافذة المنبثقة
            const streamBtnHtml = m.streamUrl ? `<button class="stream-btn" onclick="openStreamModal('${m.streamUrl}', '${m.team1} vs ${m.team2}')">📺 بث مباشر</button>` : '';
            ...
            try {
                ...
                if (isExisting) {
                    ...
                    if (existingPred) {
                        ...
                        renderUpcoming(); // تحديث لعرض "تم التوقع"
                        return;
                    }
        ...
        // دالة لجلب توقعات المستخدم وتعبئة userPredictionsMap
        async function loadUserPredictions(userName) {
            ...
            try {
                ...
                renderUpcoming(); // تحديث المباريات لإظهار علامة "تم التوقع"
            } catch (e) {
        ...
        function renderTeamStats() {
            ...
            state.previousGamesData.forEach(g => {
                ...
                if (homeScore > awayScore) { stats[homeAr].wins++;
                    stats[awayAr].losses++; } else if (awayScore > homeScore) { stats[awayAr].wins++;
        ...
        function renderBracket() {
            ...
            if (!state.openfootballMatches || !state.openfootballMatches.length) {
                container.innerHTML =
                    `<div class="empty-state"><span class="icon">⏳</span> جاري تحميل بيانات المخطط...</div>`;
                if (!isOpenfootballLoaded) {
                    fetchOpenfootballData().then(() => {
                        if (state.openfootballMatches && state.openfootballMatches.length) {
                            renderBracket();
                        } else {
                            container.innerHTML =
                                `<div class="empty-state"><span class="icon">📊</span> لا توجد بيانات للمخطط</div>`;
                        }
                    });
                }
                return;
            ...
            const roundNames = {
                'R32': { ar: 'دور الـ 32', icon: '🔵' },
                'R16': { ar: 'دور الـ 16', icon: '🟢' },
                'QF': { ar: 'ربع النهائي', icon: '🟡' },
                'SF': { ar: 'نصف النهائي', icon: '🟠' },
                '3RD': { ar: 'المركز الثالث', icon: '🥉' },
                'FINAL': { ar: 'النهائي', icon: '🏆' }
            };
            const roundMapping = {
                'Round of 32': 'R32',
                'Round of 16': 'R16',
            ...
            const roundsMap = {};
            for (let r of roundOrder) { roundsMap[r] = []; }
            const bracketMatches = state.openfootballMatches.filter(m => {
                if (m.round) {
                    const roundKey = roundMapping[m.round];
                    if (roundKey && roundOrder.includes(roundKey)) { return true; }
                }
                if (m.stage) {
                    const stage = m.stage.toLowerCase();
                    if (stage.includes('round of 32') || stage.includes('r32')) return true;
                    if (stage.includes('round of 16') || stage.includes('r16')) return true;
                    if (stage.includes('quarter')) return true;
                    if (stage.includes('semi')) return true;
                    if (stage.includes('final')) return true;
                    if (stage.includes('third')) return true;
                }
                return false;
            });
            if (!bracketMatches.length && state.allGames && state.allGames.length) {
                const fallbackGames = state.allGames.filter(g =>
                ...
                if (fallbackGames.length) {
                    for (let match of fallbackGames) {
                        let roundKey = match.group;
                        if (!roundKey || !roundOrder.includes(roundKey)) {
                            if (match.type === 'final' || match.round === 'final' || match.group === 'FINAL') roundKey =
                            'FINAL';
                            else if (match.type === 'third' || match.round === 'third' || match.group === '3RD')
                                roundKey = '3RD';
                            else continue;
                        }
                        if (roundKey && roundOrder.includes(roundKey)) {
                            roundsMap[roundKey].push(match);
                        }
                    }
                    buildBracketHTML(roundsMap);
                    return;
                } else {
                    container.innerHTML =
            ...
            for (let match of bracketMatches) {
                let roundKey = roundMapping[match.round];
                if (!roundKey) {
                    if (match.stage) {
                        const stage = match.stage.toLowerCase();
                        if (stage.includes('round of 32') || stage.includes('r32')) roundKey = 'R32';
                        else if (stage.includes('round of 16') || stage.includes('r16')) roundKey = 'R16';
                        else if (stage.includes('quarter')) roundKey = 'QF';
                        else if (stage.includes('semi')) roundKey = 'SF';
                        else if (stage.includes('final')) roundKey = 'FINAL';
                        else if (stage.includes('third')) roundKey = '3RD';
                    }
                }
                if (roundKey && roundOrder.includes(roundKey)) {
                    roundsMap[roundKey].push(match);
            ...
            function buildBracketHTML(roundsMap) {
                let html = `<div class="bracket-container">`;
                let hasMatches = false;
                for (let r of roundOrder) {
                    const matches = roundsMap[r] || [];
                    if (!matches.length) continue;
                    hasMatches = true;
                    const roundInfo = roundNames[r] || { ar: r, icon: '🏅' };
                    matches.sort((a, b) => {
                  ...
                  <div class="round-title">
                    <span class="round-icon">${roundInfo.icon}</span>
                    ${roundInfo.ar}
                    <span style="font-size:0.6rem;color:var(--text-secondary);font-weight:400;margin-right:auto;">${matches.length} مباراة</span>
                  </div>
                  <div class="round-matches">`;
                    for (let match of matches) {
                        ...
                        if (homeDisplay === homeName && !homeName.includes('متصدر') && !homeName.includes('وصيف') && !
                            homeName.includes('ثالث')) {
                            homeDisplay = translateToArabic(homeName);
                        }
                        if (awayDisplay === awayName && !awayName.includes('متصدر') && !awayName.includes('وصيف') && !
                            awayName.includes('ثالث')) {
                        ...
                        const isFinished = match.finished === true || match.finished === "TRUE" || match.status ===
                            'finished';
                        ...
                        if (isFinished) {
                            ...
                            statusHtml = `<span class="finished-status">✅ ${score}</span>`;
                            statusClass = 'finished-status';
                        } else if (isLive) {
                            statusHtml = `<span class="live-status">🔴 تُلعب الآن</span>`;
                            statusClass = 'live-status';
                        } else {
                            statusHtml = `<span class="pending-status">⏳ لم تلعب بعد</span>`;
                            statusClass = 'pending-status';
                        }
                        let matchMeta = '';
                        if (match.date || match.time) {
                            const dateStr = match.date || match.time || '';
                            matchMeta = `<div class="match-time-badge">📅 ${dateStr}</div>`;
                        }
                        const matchId = match._id || match.id || `bracket_${homeName}_${awayName}_${Date.now()}`;
                        html += `
                      <div class="bracket-match" onclick="openBracketMatchDetail('${matchId}')">
                        <div class="teams">
            ...
            try {
                ...
                if (state.previousGamesData.length === 0) {
                    document.getElementById('previousMatchesContainer').innerHTML =
                        `<div class="empty-state"><span class="icon">⚠️</span> فشل التحميل <button onclick="loadPreviousGamesFull()" style="display:block;margin:12px auto 0;background:var(--gold);border:none;padding:8px 24px;border-radius:40px;font-weight:700;color:#0a1628;cursor:pointer;font-family:inherit;">🔄 إعادة المحاولة</button></div>`;
                }
        ...
        function renderPreviousGamesFiltered() {
            ...
            if (searchText) filtered = filtered.filter(g => g.homeAr.toLowerCase().includes(searchText) || g.awayAr
        ...
        // ============================================================
        //  عرض جميع توقعات اللاعب (من الأقدم إلى الأحدث)
        // ============================================================
        ...
        // ============================================================
        //  لوحة المتصدرين (مع دعم الترتيب العام وآخر 24 ساعة)
        // ============================================================
        ...
        async function renderLeaderboard(period) {
            ...
            // تحديد المباريات بناءً على الفترة
            let filteredGames = games;
            if (period === '24h') {
                const nowTime = now();
                const last24h = nowTime - 24 * 60 * 60 * 1000;
                filteredGames = games.filter(g => {
                    const dateStr = g.formattedDate || '';
                    const timeStr = g.timeMatch || '';
                    ...
                    gameTime += 3 * 60 * 60 * 1000;
                    return gameTime >= last24h && gameTime <= nowTime;
                });
            }
            ...
            for (let p of predictions) {
                ...
                if (!match) continue;
                const result = match.homeScore > match.awayScore ? match.homeAr :
                    match.awayScore > match.homeScore ? match.awayAr :
                    "DRAW";
            ...
            // حفظ الترتيب السابق للمقارنة (أسهم الصعود والهبوط)
        ...
        function generateAnalyticsCharts() {
            ...
            for (let p of predictions) {
                ...
                if (!matchPredCount[key]) matchPredCount[key] = { home: 0, away: 0, draw: 0 };
                if (p.prediction === parts[1]) matchPredCount[key].home++;
                else if (p.prediction === parts[2]) matchPredCount[key].away++;
                else if (p.prediction === 'DRAW') matchPredCount[key].draw++;
            }

            const sortedMatches = Object.entries(matchPredCount)
                .sort((a, b) => (b[1].home + b[1].away + b[1].draw) - (a[1].home + a[1].away + a[1].draw))
                .slice(0, 8);

            const labels = sortedMatches.map(m => m[0].length > 25 ? m[0].substring(0, 22) + '...' : m[0]);
            const homeData = sortedMatches.map(m => m[1].home);
            const awayData = sortedMatches.map(m => m[1].away);
            const drawData = sortedMatches.map(m => m[1].draw);
            ...
            const popData = popularMatches.map(m => m[1].home + m[1].away + m[1].draw);

            if (chartInstancesLocal.dist) chartInstancesLocal.dist.destroy();
            ...
            const ctx1 = document.getElementById('chartDistribution').getContext('2d');
            chartInstancesLocal.dist = new Chart(ctx1, {
                type: 'bar',
                data: {
                    labels: labels,
                    datasets: [{
                        label: 'فوز الفريق الأول',
                        data: homeData,
                        backgroundColor: 'rgba(240, 180, 41, 0.7)',
                        borderColor: 'rgba(240, 180, 41, 1)',
                        borderWidth: 1
                    }, {
                        label: 'فوز الفريق الثاني',
                        data: awayData,
          ...
          `;
            ...
            setTimeout(() => {
                const ctx1b = document.getElementById('chartDistribution').getContext('2d');
                ...
                chartInstancesLocal.dist = new Chart(ctx1b, {
                    type: 'bar',
                    data: {
                        labels: labels,
                        datasets: [{
